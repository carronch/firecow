...
const STATUS_COLORS: Record<string, string> = {
    ...
    pending:   'bg-gray-100 text-slate-400 border-slate-800/60',
};
...
export default function CalendarView({ initialBookings, apiBase, adminToken }: Props) {
    ...
    return (
        <div className="flex gap-6">
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-lg font-semibold text-white w-48 text-center">{monthName}</span>
                        <button onClick={nextMonth} className="p-1.5 rounded hover:bg-gray-100"><ChevronRight size={18} /></button>
                    </div>
                    <select
                        value={filterSite}
                        onChange={e => setFilterSite(e.target.value)}
                        className="border border-slate-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-sky-500 focus:border-sky-500 bg-slate-950/50 bg-slate-900/40 backdrop-blur-md"
                    >
                ...
                <div className="bg-slate-900/40 backdrop-blur-md rounded-xl border border-slate-800/60 overflow-hidden">
                    ...
                    <div className="grid grid-cols-7 border-b border-slate-800/60">
                        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (
                            <div key={d} className="px-2 py-2 text-xs font-semibold text-slate-400 text-center uppercase">{d}</div>
                        ))}
                    </div>
                    {/* Day cells */}
                    <div className="grid grid-cols-7 divide-x divide-y divide-gray-100">
                        {cells.map((day, i) => {
                            if (!day) return <div key={i} className="min-h-[90px] bg-slate-800/30/50" />;
                            const dateStr = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
                            const dayBookings = byDate[dateStr] || [];
                            const isToday = day === today.getDate() && month === today.getMonth() && year === today.getFullYear();
                            return (
                                <div key={i} className="min-h-[90px] p-1.5 hover:bg-slate-800/30">
                                    <div className={`text-xs font-medium mb-1 w-6 h-6 flex items-center justify-center rounded-full ${isToday ? 'bg-gradient-to-r from-sky-500 to-purple-600 border-0 text-white' : 'text-slate-400'}`}>
                                        {day}
                                    </div>
            ...
            {/* Detail panel */}
            {selected && (
                <div className="w-80 flex-shrink-0 bg-slate-900/40 backdrop-blur-md rounded-xl border border-slate-800/60 p-5 self-start">
                    <div className="flex items-start justify-between mb-4">
                        ...
                        <button onClick={() => { setSelected(null); setEditData(null); }} className="text-gray-400 hover:text-slate-400 p-1">
                            <X size={16} />
                        </button>
                    </div>
                        ...
                        <div className="space-y-2 text-xs">
                            ...
                            <div className="flex gap-2 pt-1">
                                ...
                                <button onClick={() => setEditData(null)}
                                    className="flex items-center gap-1 px-3 py-1.5 border border-slate-700 text-xs rounded-lg hover:bg-slate-800/30">
                                    <X size={12} /> Cancel
                                </button>
                            </div>
                        </div>
                        ...
                        <>
                            <dl className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <dd className="text-xs text-slate-400">{selected.site_id || '—'}</dd>
                                </div>
                                {selected.notes && (
                                    <div className="pt-1 border-t border-gray-100">
                                        <dt className="text-slate-400 text-xs mb-0.5">Notes</dt>
                                        <dd className="text-xs text-slate-400 break-words">{selected.notes}</dd>
                                    </div>
                                )}
                            </dl>
                            <button
                                ...
                                className="mt-4 flex items-center gap-1.5 px-3 py-1.5 border border-slate-700 text-sm rounded-lg hover:bg-slate-800/30 w-full justify-center"
                            >
